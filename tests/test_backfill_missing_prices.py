...
def test_backfill_missing_prices_workflow(mocker):
    ...
    # 2. Mock configuration loading and sleep duration
    mocker.patch.object(backfill_module, "config", MOCK_CONFIG)
    mocker.patch("time.sleep", return_value=None)

    # 3. Mock fetch_tiingo_data on the imported module
    ...
    mocker.patch.object(
        backfill_module, "fetch_tiingo_data", return_value=fake_backfilled_df
    )

    # 4. Mock file reading strictly for open() inside backfill_missing_prices
    mock_file_data = json.dumps(
        [{"Ticker": "FRC", "CIK": "0000000000", "Reason": "Test reason"}]
    )
    mocker.patch.object(
        backfill_module, "open", mocker.mock_open(read_data=mock_file_data), create=True
    )

    # 5. Mock selective path checks
    def mock_exists(path):
        return path in ["fake_missing.json", "fake_prices.csv"]

    mocker.patch("os.path.exists", side_effect=mock_exists)

    # 6. Mock reading existing prices CSV
    mocker.patch("pandas.read_csv", return_value=MOCK_EXISTING_PRICES.copy())

    # 7. Mock saving to CSV
