...
MOCK_CONFIG = {
    "paths": {
        "events_ledger": "fake_path_events.csv"
    }
}
...
MOCK_HISTORICAL_CHANGES = pd.DataFrame({
    ...
    "Removed Ticker": ["A", "B", pd.NA], # The last row has NaN in Removed Ticker (addition only)
...
def test_generate_corporate_events(mocker):
    ...
    mocker.patch('s_and_p_500_picker.generate_corporate_events.json.load', return_value=MOCK_CONFIG)
    ...
    assert result_df.iloc[0]["Event Type"] == "SPIN_OFF"   # Map from "DOW spun off from DWDP"
    assert result_df.iloc[1]["Event Type"] == "BANKRUPTCY" # Map from "Filing for bankruptcy protection"
