...
def test_process_company_fundamentals(mocker):
    ...
    mocker.patch('s_and_p_500_picker.fetch_sec_fundamentals.fetch_company_facts', return_value=MOCK_SEC_JSON)
...
def test_generate_fundamentals_dataset(mocker):
    ...
    valid_df = pd.DataFrame({
        "Filing Date": ["2022-10-28"], "Period End": ["2022-09-24"], 
        "Form": ["10-K"], "Revenue": [1000], "CIK": ["0000320193"], "Ticker": ["AAPL"]
    })
    ...
    mocker.patch('s_and_p_500_picker.fetch_sec_fundamentals.process_company_fundamentals', side_effect=mock_process)
