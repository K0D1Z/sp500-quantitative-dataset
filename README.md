...
8. **Create only one accout:** To use the dataset, you create only one account.
...
* **Tiingo API Key:** Free account at [Tiingo](https://api.tiingo.com/) for historical delisted price backfilling. If you don't want to use your real email address, just simply create an additional one with a service provider like [Tuta](https://tuta.com/) (temporary email addresses may not work).
...
Loading the dataset using **Pandas** or **Polars** to analyze historical valuation multiples:
...
print(aapl[["Date", "Close", "Market Cap", "P/E Ratio", "RSI_14", "Volatility_30D"]].head())
