...
def fetch_company_facts(cik: str) -> dict:
    ...
    if response.status_code == 200:
        ...
        print(f"Failed to fetch SEC data for CIK {padded_cik}. Status code: {response.status_code}")
        return {}

def extract_metric_dataframe(facts_data: dict, tag_name: str, metric_label: str) -> pd.DataFrame:
    """
    ...
    try:
        ...
        df = df.sort_values(by="Filing Date").drop_duplicates(subset=["Period End", "Form"], keep="last")
...
def process_company_fundamentals(cik: str, ticker: str) -> pd.DataFrame:
    ...
    metrics_mapping = {
        ...
        "R&D Expenses": ["ResearchAndDevelopmentExpense", "ResearchAndDevelopmentExpenseSoftwareExcludingAcquiredInProcessCost"],
        ...
        "Cash and Equivalents": ["CashAndCashEquivalentsAtCarryingValue", "CashAndCashEquivalentsFairValueDisclosure"],
        ...
        "Operating Cash Flow": ["NetCashProvidedByUsedInOperatingActivities", "NetCashProvidedByUsedInOperatingActivitiesContinuingOperations", "OperatingCashFlow"],
        "CapEx": ["PaymentsToAcquirePropertyPlantAndEquipment", "PaymentsToAcquireProductiveAssets"],
        "Dividends Paid": ["PaymentsOfDividends", "PaymentsOfDividendsCommonStock"],
        "Stock Repurchases": ["PaymentsForRepurchaseOfCommonStock", "PaymentsForRepurchaseOfEquity"],
        
        # --- SHARE COUNTS (Valuation Multiples Baseline) ---
        "Shares Outstanding (Basic)": ["WeightedAverageNumberOfSharesOutstandingBasic", "CommonStockSharesOutstanding", "EntityCommonStockSharesOutstanding"],
    ...
    for next_df in dfs[1:]:
        base_df = pd.merge(base_df, next_df, on=["Filing Date", "Period End", "Form"], how="outer")
...
def generate_fundamentals_dataset() -> None:
    ...
    csv_output_path = config["paths"].get("fundamentals_csv", "data/s_and_p_500_fundamentals.csv")
    parquet_output_path = config["paths"].get("fundamentals_parquet", "data/s_and_p_500_fundamentals.parquet")
    failed_sec_path = config["paths"].get("failed_sec_log", "data/failed_sec_fundamentals.json")

    if not os.path.exists(composition_path):
        print(f"Composition file not found at {composition_path}. Run daily composition script first.")
        return

    df_comp = pd.read_csv(composition_path)
    unique_entities = df_comp[["Ticker", "CIK"]].dropna().drop_duplicates().values

    print(f"Starting fundamentals extraction for {len(unique_entities)} unique entities from SEC EDGAR...")
    ...
    master_df = master_df.sort_values(by=["Ticker", "Filing Date"]).reset_index(drop=True)
