...
def load_and_clean_prices(filepath: str) -> pd.DataFrame:
    """
    Loads the historical price data.
    """
    if not os.path.exists(filepath):
...
def calculate_financial_ratios(df: pd.DataFrame) -> pd.DataFrame:
    ...
    shares = df["Shares Outstanding (Diluted)"].fillna(df["Shares Outstanding (Basic)"])
    df["Market Cap"] = df["Close"] * shares

    # Price-to-Earnings (P/E) Ratio
    # Using EPS Diluted directly if available
    df["P/E Ratio"] = np.where(
        (df["EPS (Diluted)"].notna()) & (df["EPS (Diluted)"] > 0),
        df["Close"] / df["EPS (Diluted)"],
        np.nan
    )

    # Price-to-Book (P/B) Ratio
    # Book Value Per Share = Stockholders Equity / Shares Outstanding
    book_value_per_share = df["Stockholders Equity"] / shares
