...
def map_ticker_to_cik() -> pd.DataFrame:
    ...
    sec_tickers = sec_tickers.rename(
        columns={"cik_str": "CIK", "ticker": "Ticker", "title": "Company Name"}
    )
    ...
    removed_base["Company Name"] = removed_base["Company Name"].fillna(
        removed_base["Wiki Company Name"]
    )
    ...
    missing_ciks_after_fallback = final_mapping[
        final_mapping["Ticker"].isna()
        | final_mapping["CIK"].isna()
        | final_mapping["Company Name"].isna()
    ]
    if not missing_ciks_after_fallback.empty:
        print(
            "WARNING: The following historical tickers are still missing CIKs and need to be added to fallback_ciks.json:"
        )
